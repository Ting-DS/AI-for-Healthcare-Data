...
def get_predicted_volumes(pred):
    ...
    volume_ant = pred[pred==1].sum()
    volume_post = pred[pred==2].sum()/2
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text((10, 90),
                        (f"Patient ID: {header.PatientID}\nPatient's Name: {header[0x0010,0x0010].value}\nSeries Description: {header[0x0008,0x103E].value}\n"+  # Body Part examined is not required
                         f"Volume ant: {inference['anterior']}\nVolume post: {inference['posterior']}\nTotal volume: {inference['total']}"), 
    ...
    # Try to figure out what dimension we can use to maximize the size of the images while keeping their ratio intact
    resize_factors = (min(980/orig_vol.shape[0]/3, 710/orig_vol.shape[1]), min(710/orig_vol.shape[1]/3, 980/orig_vol.shape[0]))
    max_dim = 0  # dimension to put the images on to maximize their sizes
    if resize_factors[1]>resize_factors[0]:
        max_dim = 1
    print(max_dim)
        
    dimensions = (int(orig_vol.shape[0]*resize_factors[max_dim]), int(orig_vol.shape[1]*resize_factors[max_dim]))
    
    for i, sn in enumerate(slice_nums):
        orig_slice = orig_vol[:,:,sn]
        pred_slice = pred_vol[:,:,sn]
        orig_nd_img = np.flip((orig_slice/np.max(orig_slice))*0xff).T.astype(np.uint8)
        ...
        pil_i_orig = Image.fromarray(orig_nd_img, mode="L").convert("RGBA").resize(dimensions)
        pil_i_pred = Image.fromarray(pred_nd_img, mode="L").convert("RGBA").resize(dimensions)
        # Create the blended image with the prediction
        pil_i = Image.blend(pil_i_orig, pil_i_pred, 0.5)
        # Paste the PIL image into our main report image object (pimg)
        # We add a tuple to the starting 'box': (10,280) to increment the coordinates of the image
        box_incr = [10, 280]
        box_incr[max_dim] += i*dimensions[max_dim]
        pimg.paste(pil_i, box=tuple(box_incr))
...
def get_series_for_inference(path):
    ...
    # Series Description is HippoCrop
    
    series_for_inference = [dicom for dicom in dicoms if 'HippoCrop' in dicom[0x0008,0x103E].value]
...
if __name__ == "__main__":
    ...
    # Get the latest directory (not really used, I deleted the other directories so that this works as per https://knowledge.udacity.com/questions/280858)
    ...
    report_save_path = r"reports/report.dcm"
