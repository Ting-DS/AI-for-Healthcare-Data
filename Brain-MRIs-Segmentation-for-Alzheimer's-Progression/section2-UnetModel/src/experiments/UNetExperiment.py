...
class UNetExperiment:
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            data = batch["image"].to(self.device, dtype=torch.float)  # torch.from_numpy()
            ...
            # ANSWER: prediction is of size [1, 3, 64, 64]
            # 1 is the size of the batch
            # Unet is set up with num_classes=3 so it's a prediction probability for each class for each pixel
            # 64, 64 is the size of the images
    ...
    def run_test(self):
        ...
        for i, x in enumerate(self.test_data):
            ...
            # use binary masks for the dice score and the other metrics
            ...
            dc = Dice3d(pred_label, seg_label)
            jc = Jaccard3d(pred_label, seg_label)
            sens = sensitivity(pred_label, seg_label)
            spec = specificity(pred_label, seg_label)
            dc_list.append(dc)
            jc_list.append(jc)
